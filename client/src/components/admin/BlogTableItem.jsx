...
const BlogTableItem = ({blog, fetchBlogs, index}) => {
  ...
  const deleteBlog = async () => {
    ...
    if(!confirm) return;
    try {
      const {data} = await axios.post('/api/admin/delete-blog', {id: blog._id}, adminAxiosConfig())
      if(data.success) {
  ...
  const togglePublish = async () => {
    try {
      const {data} = await axios.post('/api/admin/toggle-publish', {id: blog._id}, adminAxiosConfig())
      if(data.success) {
        toast.success(data.message)
        await fetchBlogs()
      } else {
