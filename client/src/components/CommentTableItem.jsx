...
const CommentTableItem = ({comment, fetchComments}) => {
  ...
  const approveComment = async () => {
    try {
      const {data} = await axios.post('/api/admin/approve-comment', {id: _id}, adminAxiosConfig())
      if(data.success) {
  ...
  const deleteComment = async () => {
    try {
      ...
      if(!confirm) return;
      const {data} = await axios.post('/api/admin/delete-comment', {id: _id}, adminAxiosConfig())
      if(data.success) {
        toast.success(data.message)
        fetchComments()
      } else {
